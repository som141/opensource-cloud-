...
public class PresignedUploadService {
    ...
    private void validateRequest(UploadSession uploadSession, List<PresignedUploadFileRequest> files) {
        long issuedFileCount = uploadSessionFileRepository.countByUploadSessionId(uploadSession.getId());
        if (issuedFileCount + files.size() > uploadSession.getExpectedFileCount()) {
        ...
        long issuedSizeBytes = uploadSessionFileRepository.sumSizeBytesByUploadSessionId(uploadSession.getId());
        if (issuedSizeBytes + totalSizeBytes > uploadSession.getExpectedTotalSizeBytes()) {
