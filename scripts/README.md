...
| `generate-keda-comparison-report.py` | KEDA ON/OFF 500장 배치 결과 JSON, Markdown, PDF 리포트를 생성한다. |
| `generate-autoscaling-comparison-report.py` | Fixed 1, HPA CPU, KEDA min 1, KEDA min 0 비교 리포트를 생성한다. |
...
Kubernetes 기본 HPA CPU 기준 실험은 metrics-server가 설치된 상태에서 다음과 같이 실행한다.

```powershell
.\scripts\k8s-scale-mode.ps1 -Mode hpa-cpu -HpaMinReplicas 1 -HpaMaxReplicas 20 -TargetCpuUtilization 60 -KubeConfig "C:\path\to\kube.conf"
```

KEDA를 HPA와 같은 `min 1` 출발선에서 비교하려면 다음 모드를 사용한다.

```powershell
.\scripts\k8s-scale-mode.ps1 -Mode keda-on-min1 -KubeConfig "C:\path\to\kube.conf"
```
...
KEDA ON/OFF 실험이 끝난 뒤 비교 리포트는 다음 명령으로 다시 생성한다.

```powershell
python .\scripts\generate-keda-comparison-report.py
```

결과는 `benchmark-results/20260601-keda-comparison-report.pdf`에 저장된다.

Fixed 1, HPA CPU, KEDA min 1, KEDA min 0을 모두 비교하는 리포트는 다음 명령으로 생성한다.

```powershell
python .\scripts\generate-autoscaling-comparison-report.py
```

결과는 `benchmark-results/20260601-autoscaling-comparison-report.pdf`에 저장된다.
