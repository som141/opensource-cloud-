...
대규모 스캔 문서 이미지를 OCR에 적합한 형태로 전처리하는 비동기 SaaS 플랫폼입니다.
이 프로젝트는 단순 이미지 리사이징 서비스가 아니라, Spring Boot API와 RabbitMQ Worker를 분리해 대량 문서 이미지를 안정적으로 처리하는 파이프라인을 목표로 합니다.

## 핵심 가치

- **대량 처리**: 여러 이미지 또는 ZIP 파일을 업로드하면 이미지 단위 JobItem으로 분리해 큐 기반으로 처리합니다.
- **OCR 전처리 전용**: Worker는 decode, 색상 정규화, 방향 보정, deskew, crop, denoise, contrast normalize, binarization, morphology cleanup, DPI normalize, sharpen 단계를 수행합니다.
- **API와 Worker 분리**: Spring REST API는 인증, 프로젝트, 업로드, Job 등록/조회만 담당하고, 이미지 전처리는 Worker가 수행합니다.
...
| 업로드 | 이미지 다중 업로드, ZIP 업로드, presigned URL 기반 Object Storage 업로드 |
...
Google OAuth를 실제로 테스트하려면 `.env`에 Google Client ID/Secret을 넣고 Google Console에 아래 redirect URI를 등록합니다.
...
Codex/AI 작업 규칙 문서인 `AGENTS.md`, `CODEX_DIRECTORY_SPEC.md`, `LOCAL_CONFIG.md`는 로컬 전용 문서로 관리하며 Git 추적 대상에서 제외합니다.
...
- Bootstrap, jQuery, AdminLTE 같은 임의 템플릿은 추가하지 않습니다.
- 기능 변경 PR은 관련 문서와 검증 결과를 함께 남깁니다.
