...
현재 권장 비교 시나리오는 아래 4개다. KEDA의 장점을 입증하려면 단순히 `KEDA ON`과 `Worker 1개 고정`만 비교하지 말고, Kubernetes 기본 HPA CPU 방식도 함께 비교한다.

| 시나리오 | 설명 | 목적 |
| --- | --- | --- |
| `fixed-1` | KEDA 비활성화, Worker 1개 고정 | 가장 단순한 기준값 |
| `hpa-cpu` | KEDA 비활성화, CPU 사용률 기준 HPA | Kubernetes 기본 autoscaling 비교 |
| `keda-min-1` | KEDA 활성화, Worker 1~20개 자동 확장 | 빠른 응답과 큐 기반 확장 비교 |
| `keda-min-0` | KEDA 활성화, Worker 0~20개 자동 확장 | scale-to-zero 비용 절감 운영 방식 |
...
- KEDA min 1은 HPA CPU보다 처리 구간 기준 1.54배 빠르다.
- KEDA min 0은 유휴 비용은 가장 낮지만 cold start 대기 시간이 포함된다.
- HPA CPU는 queue backlog를 직접 보지 못하기 때문에 RabbitMQ 기반 batch Worker에는 KEDA보다 반응이 늦다.
